...
def get_packages(
    ...
    if plan_type:
        query = query.filter(Package.plan_type == plan_type)
...
def get_package_stats(db: Session = Depends(get_db)):
    ...
    plan_types = db.query(func.count(func.distinct(Package.plan_type))).scalar()
...
# ==========================================================
# PACKAGES WITH SUBSCRIBER COUNTS
# ==========================================================
...
def create_package(package: PackageCreate, db: Session = Depends(get_db)):
    ...
    existing = (
        db.query(Package)
        .filter(Package.name == package.name, Package.plan_type == package.plan_type)
        .first()
    )
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail=f"Package '{package.name}' with plan type '{package.plan_type}' already exists",
        )

    # ✅ Directly save the package (modules is a string)
...
def update_package(
    ...
    # Check uniqueness if name or plan_type is being updated
    if "name" in update_data or "plan_type" in update_data:
        new_name = update_data.get("name", db_package.name)
        new_plan_type = update_data.get("plan_type", db_package.plan_type)

        existing = (
            db.query(Package)
            .filter(
                Package.name == new_name,
                Package.plan_type == new_plan_type,
        ...
        if existing:
            raise HTTPException(
                status_code=400,
                detail=f"Package '{new_name}' with plan type '{new_plan_type}' already exists",
            )
